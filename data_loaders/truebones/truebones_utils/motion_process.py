...
def safe_normalize_vectors(vectors, fallback=np.array([0, 0, 1]), eps=1e-8):
    ...
    valid = norms > eps
    normalized[valid] = normalized[valid] / norms[valid, np.newaxis]
    normalized[~valid] = fallback
    return normalized

def yaw_quat_to_face_z(forward):
...
hip-shoulder convention. Two indices are treated as a tail-to-head/body-centerline
direction, which is more stable for Planet Zoo quadrupeds than swinging leg pairs.
face_joint_indx is optional for object_types that are not included in FACE_JOINTS dict"""
def get_root_quat(joints, object_type, face_joint_indx=None):
    ...
    if len(face_joint_indx) == 2:
        ...
        forward = joints[:, head] - joints[:, tail]
        forward[:, 1] = 0
        forward = safe_normalize_vectors(forward)
        ...
        across = safe_normalize_vectors(across1 + across2, fallback=np.array([1, 0, 0]))
        forward = np.cross(np.array([[0, 1, 0]]), across, axis=-1)
        forward = safe_normalize_vectors(forward)
    else:
        raise ValueError("face_joint_indx must contain either 2 centerline joints or 4 left/right joints")
    root_quat = yaw_quat_to_face_z(forward)
...
def put_on_ground(anim, ground_height=None, ground_joint_indices=None):
    if ground_height is None:
        ...
        if ground_joint_indices is not None and len(ground_joint_indices) > 0:
            ground_height = t_pos_global_positions[:, ground_joint_indices, 1].min()
        else:
...
def apply_planetzoo_global_roll(anim, object_type):
    if not object_type.startswith("PZ_"):
        return anim
    angle = np.deg2rad(PLANETZOO_GLOBAL_ROLL_Z_DEG)
    qs_roll = Quaternions.from_angle_axis(
        np.full(anim.rotations.shape[0], angle),
        np.array([0, 0, 1]),
    )
    new_rots = anim.rotations.copy()
    new_rots[:, 0] = qs_roll * new_rots[:, 0]
    new_pos = anim.positions.copy()
    new_pos[:, 0] = qs_roll * new_pos[:, 0]
    return Animation(new_rots, new_pos, anim.orients.copy(), anim.offsets.copy(), anim.parents.copy())

""" scale skeleton s.t mean reference bone length is HML_AVG_BONELEN """
def scale(anim, scale_factor=None, scale_joint_indices=None):
    if scale_factor is None:
        if scale_joint_indices is not None and len(scale_joint_indices) > 0:
            lengths = np.linalg.norm(anim.offsets[scale_joint_indices], axis=1)
...
""" get foot contact """
def get_foot_contact(positions, foot_joints_indices, vel_thresh, relative_ground=False):
    ...
    foot_height = positions[1:, foot_joints_indices, 1]
    if relative_ground and len(foot_joints_indices) > 0:
        foot_height = foot_height - positions[:, foot_joints_indices, 1].min()
    foot_contact_vel_map = np.where(np.logical_and(total_vel <= vel_thresh, np.abs(foot_height) <= FOOT_CONTACT_HEIGHT_THRESH), 1, 0)
...
def get_planetzoo_scale_joint_indices(joint_names, offsets):
    indices = []
    include_tokens = [
        "spine2",
        "spine3",
    ...
    exclude_tokens = ["end_site", "twist", "srb", "breath"]
    for idx, name in enumerate(joint_names):
        lname = name.lower()
        if idx == 0 or np.linalg.norm(offsets[idx]) <= 1e-8:
            continue
        if any(token in lname for token in exclude_tokens):
            continue
        if any(token in lname for token in include_tokens):
            indices.append(idx)
    return indices

def get_planetzoo_ground_joint_indices(joint_names):
    return [
        idx
        for idx, name in enumerate(joint_names)
        if any(token in name.lower() for token in ["toe", "foot", "hoof", "ashi"])
    ]

def get_scale_and_ground_indices(object_type, joint_names, offsets):
    if object_type.startswith("PZ_"):
        return get_planetzoo_scale_joint_indices(joint_names, offsets), get_planetzoo_ground_joint_indices(joint_names)
    return None, None

"""" process anim object """
def process_anim(anim, object_type, root_pose_init_xz=None, scale_factor=None, ground_height=None, face_joints=None, scale_joint_indices=None, ground_joint_indices=None):
    rotated = rotate_to_hml_orientation(anim, object_type, face_joints) 
    rolled = apply_planetzoo_global_roll(rotated, object_type)
    centered, root_pose_init_xz_ = move_xz_to_origin(rolled, root_pose_init_xz)
    scaled, scale_factor_ = scale(centered, scale_factor, scale_joint_indices)
    grounded, ground_height_ = put_on_ground(scaled, ground_height, ground_joint_indices)
    return grounded, root_pose_init_xz_, ground_height_, scale_factor_

def has_zero_length_offsets(anim, eps=1e-8):
    if len(anim.offsets) <= 1:
        return False
    return np.any(np.linalg.norm(anim.offsets[1:], axis=1) < eps)

def is_valid_single_tree_anim(anim):
    return (
        np.isfinite(anim.positions).all()
        and np.isfinite(anim.rotations.qs).all()
        and np.isfinite(anim.offsets).all()
        and np.count_nonzero(anim.parents == -1) == 1
        and not np.any(anim.parents < -1)
    )

def prune_planetzoo_helper_joints(anim, joint_names, object_type):
    if not object_type.startswith("PZ_"):
        return anim, joint_names
    excluded = {i for i, name in enumerate(joint_names) if name.lower() == "srb" or name.lower().startswith("srb_")}
    if not excluded:
        return anim, joint_names
    changed = True
    while changed:
        changed = False
        for idx, parent in enumerate(anim.parents):
            if parent in excluded and idx not in excluded:
                excluded.add(idx)
                changed = True
    keep_indices = [idx for idx in range(len(joint_names)) if idx not in excluded]
    print(f"INFO: pruned {len(excluded)} Planet Zoo helper joints: " + ", ".join(joint_names[i] for i in sorted(excluded)))
    return anim[:, keep_indices], [joint_names[i] for i in keep_indices]
...
def get_common_features_from_T_pose(t_pose_bvh, object_type, face_joints=None):
    ...
    t_pose_anim, t_pos_names = prune_planetzoo_helper_joints(t_pose_anim, t_pos_names, object_type)
    if face_joints:
    ...
    # Full-topology game skeletons often contain zero-length helper/twist joints; IK can produce NaNs or invalid parents there.
    if has_zero_length_offsets(t_pose_anim):
        print(f"WARNING: skipping T-pose IK for {object_type}; zero-length offsets detected.")
    else:
        t_pose_positions = positions_global(t_pose_anim)
        ik_t_pose_anim, _1, _2 = animation_from_positions(positions=t_pose_positions, parents=t_pose_anim.parents, offsets=t_pose_anim.offsets, iterations=150)
        if is_valid_single_tree_anim(ik_t_pose_anim):
            t_pose_anim = ik_t_pose_anim
        else:
            print(f"WARNING: skipping T-pose IK for {object_type}; IK result is not finite or not a single tree.")
    ground_height=None
    if object_type == "Dragon":
        ground_height=0
    scale_joint_indices, ground_joint_indices = get_scale_and_ground_indices(object_type, t_pos_names, t_pose_anim.offsets)
    if scale_joint_indices is not None:
        print(f"INFO: using {len(scale_joint_indices)} Planet Zoo scale reference joints for {object_type}.")
    if ground_joint_indices is not None:
        print(f"INFO: using {len(ground_joint_indices)} Planet Zoo ground reference joints for {object_type}.")
    scaled, root_pose_init_xz, ground_height, scale_factor = process_anim(
        t_pose_anim,
        object_type,
        ground_height=ground_height,
        face_joints=face_joints,
        scale_joint_indices=scale_joint_indices,
        ground_joint_indices=ground_joint_indices,
    )
...
def get_motion(bvh_path, foot_contact_vel_thresh, object_type, max_joints,root_pose_init_xz, scale_factor, ground_height, offsets, foot_indices, tpos_rots, squared_positions_error, face_joints=None, slice_inds=None, rest_pose_align_quat=None):
    try:
        ...
        foot_contact = get_foot_contact(global_positions, foot_indices, foot_contact_vel_thresh, relative_ground=object_type.startswith("PZ_"))
